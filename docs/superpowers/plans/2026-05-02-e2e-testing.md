...
**Goal:** Add Playwright E2E tests that validate integration between the Next.js frontend and the Rust/Axum backend, covering all 9 dashboard pages (health, API keys, credentials, proofs, events, usage, counterparties/issuer status, policies, team).
...
frontend/
  ...
  e2e/
    health.spec.ts               — Dashboard loads, sidebar renders, console error check
    api-keys.spec.ts             — Create, list, revoke API keys
    credentials.spec.ts          — Lookup wallet credential, issue, revoke
    proofs.spec.ts               — Search attestation, view proof cards, roots fetch
    events.spec.ts               — Audit log table, filters, pagination, events fetch
    usage.spec.ts                — Billing cards, usage chart, invoices, usage fetch
    counterparties.spec.ts       — Issuer status, Merkle roots, publish roots
    policies.spec.ts             — Policies page (coming soon scaffold)
    team.spec.ts                 — Team page (coming soon scaffold)
